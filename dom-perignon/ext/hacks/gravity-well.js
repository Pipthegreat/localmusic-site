...
//
// Replaces the older "Floor is Lava" hijink. Differences:
//   - Much slower fall (gravity 0.05 vs 0.45) — feels like a slow sinking
//   - No lava band at the bottom; pieces just pile against the viewport floor
//   - Periodically re-scans for new elements so the page continues piling
//     up indefinitely (catches SPA renders, lazy-loaded images, etc.)
//   - Column-shelf stacking so pieces rest on top of pieces that landed
//     before them, instead of all overlapping on the floor
//
// Click any falling piece to rescue it back to its original spot. The page
// DOM is never mutated — clones float over the originals (whose visibility
// is hidden once the clone takes off, restored on rescue or teardown).
...
(function () {
  ...
  const ITEMS_PER_SCAN     = 6;  // cap per scan so the pile-up is gradual
  const COLUMNS = 48;            // shelf resolution (higher = neater stacking)
  ...
  // Lowest shelf-top across the columns this item spans = highest landing point
  ...
  function step(t) {
    for (const it of fallingItems) {
      ...
      // Slow gravity, capped at terminal velocity
      it.vy = Math.min(it.vy + GRAVITY, TERMINAL_V);
      it.y += it.vy;

      // Land on the shelf for this column range
  ...
  function rescan() {
    ...
    // Process up to ITEMS_PER_SCAN new items this tick — so the pile grows
    // gradually instead of dumping everything at once on big pages.
    const batch = found.slice(0, ITEMS_PER_SCAN);
  ...
  async function init(r) {
    ...
    // Info chip — small, top-center
    chipEl = document.createElement('div');
    chipEl.className = 'dp-gw-chip';
    chipEl.textContent = 'EVERYTHING IS FALLING · CLICK TO RESCUE';
    root.appendChild(chipEl);

    // Wait a moment for the page to settle before the first scan (so img
    // elements have measured bounding rects and we don't miss them)
    setTimeout(rescan, 250);
    scanT = setInterval(rescan, RESCAN_INTERVAL_MS);
    raf = requestAnimationFrame(step);

    // If the viewport ever resizes, re-baseline the shelves so the pile
    // tracks the new floor. (Existing settled items stay where they were.)
    window.addEventListener('resize', onResize);
  }

  function onResize() {
    // Reset shelves to the new viewport floor, then re-claim by settled items
    const vh = window.innerHeight;
  ...
  function teardown() {
    ...
    // Restore every original we hid, and clear the processed flags so a
    // re-activation can pick the elements up again.
    for (const it of fallingItems) {
